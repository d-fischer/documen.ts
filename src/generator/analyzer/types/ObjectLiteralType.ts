...
export const objectLiteralTypeReflector: TypeReflector<ts.TypeLiteralNode> = {
	...
	async fromNode(checker, node) {
		...
		const members = await resolvePromiseArray(checker.getPropertiesOfType(type).map(async prop => createReflection(checker, prop, symbol)));
		const signatures = await resolvePromiseArray(type.getCallSignatures().map(async sig => SignatureReflection.fromTsSignature(checker, '__type', ts.SyntaxKind.CallSignature, sig)));
	...
	async fromType(checker, type) {
		...
		const members = await resolvePromiseArray(checker.getPropertiesOfType(type).map(async prop => createReflection(checker, prop, type.symbol)));
		const signatures = await resolvePromiseArray(type.getCallSignatures().map(async sig => SignatureReflection.fromTsSignature(checker, '__type', ts.SyntaxKind.CallSignature, sig)));
